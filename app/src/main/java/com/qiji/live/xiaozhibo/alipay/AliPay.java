package com.qiji.live.xiaozhibo.alipay;
...
public class AliPay{
    ...
    public void initPay(String orderId,String money,String num){

        final  String subject = num + "金币";
        final String body = num +"金币";
        final String total_fee = money;
        String uid = UserInfoMgr.getInstance().getUid();
        //服务器异步通知页面路径,需要自己定义  参数 notify_url，如果商户没设定，则不会进行该操作
        final String url = TCConstants.SVR_POST_URL + "/Appapi/Pay/notify_ali";
        //获取订单号码


        mOut_trade_no = orderId;
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultObj.getResultStatus(), "9000")) {
                        ...
                        Toast.makeText(mPayActivity,"支付成功",Toast.LENGTH_LONG).show();
                        ...
                        if (TextUtils.equals(resultObj.getResultStatus(), "8000")) {
                            Toast.makeText(mPayActivity,"支付结果确认中",Toast.LENGTH_LONG).show();

                        } else {
                            Toast.makeText(mPayActivity,"支付失败",Toast.LENGTH_LONG).show();

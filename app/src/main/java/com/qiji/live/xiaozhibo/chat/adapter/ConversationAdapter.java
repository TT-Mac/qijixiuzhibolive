package com.qiji.live.xiaozhibo.chat.adapter;
...
import com.qiji.live.xiaozhibo.utils.TimeUtil;
 ...
 * 会话界面adapter
 */
public class ConversationAdapter extends ArrayAdapter<ConversationUserJson> {
     ...
     */
    public ConversationAdapter(Context context, int resource, List<ConversationUserJson> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final ConversationUserJson data = getItem(position);
        viewHolder.tvName.setText(data.user_nicename);
        TCUtils.showPicWithUrl(TCApplication.getInstance(),viewHolder.avatar,data.avatar_thumb,R.drawable.bg);
        //viewHolder.avatar.setImageResource(data.getAvatar());
        viewHolder.lastMessage.setText(data.conversation.getLastMessageSummary());
        viewHolder.time.setText(TimeUtil.getTimeStr(data.conversation.getLastMessageTime()));
        long unRead = data.conversation.getUnreadNum();
        if (unRead <= 0){
